...
// Migration: add `competitionMode` and `playoffTeams` to Room (Phase 12a H2H).
//
// The immutable competition configuration is set once at room creation and
// never changes: `competitionMode` discriminates #Cumulative vs #HeadToHead
// Best Ball scoring, and `playoffTeams` is the number of teams advancing to
// the playoffs (0 for cumulative / no-playoff rooms, 4/6/8 for H2H playoffs).
// Both are irrelevant for #Auction rooms (accepted and ignored at creation).
//
// OldActor matches the deployed .most signature exactly — which equals the
// NewActor of the frozen 20260906_120000.mo migration (the source of truth).
// That shape has Room WITHOUT competitionMode/playoffTeams and
// BestBallConfig = { startWeek } only.
//
// NewActor is identical except Room gains the two new fields. The migration
// body maps every existing room through the shape change, backfilling
// competitionMode = #Cumulative and playoffTeams = 0 — exactly the values
// createRoom would have produced for every pre-existing room (all pre-H2H
// rooms are cumulative Best Ball or auction, so #Cumulative/0 is correct).
//
...
module {
  ...
  // Old types (deployed .most shape — the NewActor of 20260906_120000.mo).
  ...
  // BestBallConfig — carries only startWeek (the end is the FINAL_WEEK
  // constant, never stored).
  type BestBallConfig = {
    startWeek : Nat;
  };

  // Old Room — the deployed .most shape (no competitionMode/playoffTeams yet).
  type OldRoom = {
    id : RoomId;
    name : Text;
    admin : UserId;
  ...
  // Competition mode — chosen once at room creation and immutable forever after.
  type CompetitionMode = {
    #Cumulative;
    #HeadToHead;
  };

  // New Room — gains competitionMode and playoffTeams.
  type NewRoom = {
    id : RoomId;
    name : Text;
    admin : UserId;
  ...
  // This is the NewActor of the frozen 20260906_120000.mo migration — the
  // source of truth for the deployed signature. Room has no competitionMode.
  type OldActor = {
  ...
  // ── NewActor: Room gains competitionMode and playoffTeams ────────────────
  ...
  public func migration(old : OldActor) : NewActor {
    // Backfill every existing room with competitionMode = #Cumulative and
    // playoffTeams = 0 — the values createRoom would have produced for all
    // pre-H2H rooms (cumulative Best Ball or auction). No other state changes.
    let rooms = old.rooms.map<RoomId, OldRoom, NewRoom>(
      func(_roomId, room) {
        {
          room with
