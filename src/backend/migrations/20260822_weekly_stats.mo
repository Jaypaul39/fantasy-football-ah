...
// Migration: weekly player stats storage (Phase 1).
//
// This is an additive-field migration for the weekly-stats domain:
//
//   weeklyPlayerStats : Map.Map<Text, WeeklyPlayerStats> — raw weekly player
//   statistics keyed by a composite Text key
//     playerId # "|" # Nat.toText(season) # "|" # Nat.toText(week)
//   seeded empty for existing installs. Raw stat categories only — NO
//   precomputed points (ptsStd/ptsHalfPpr/ptsPpr are computed in a later phase,
//   never stored redundantly).
//
// OldActor matches the deployed .most signature exactly — which equals the
// NewActor of the frozen 20260822_000000.mo migration (the source of truth).
// That shape has season/scoringFormat/bestBallConfigs and includes
// draftedPlayerIds. weeklyPlayerStats is the only new field.
//
...
module {
  ...
  // Old types (deployed .most shape — the NewActor of 20260822_000000.mo).
  ...
  // This is the NewActor of the frozen 20260822_000000.mo migration — the
  // source of truth for the deployed signature. It has season/scoringFormat/
  // bestBallConfigs and includes draftedPlayerIds. No weeklyPlayerStats.
  type OldActor = {
  ...
  // ── NewActor: adds weeklyPlayerStats ─────────────────────────────────────
  //   - weeklyPlayerStats : Map.Map<Text, WeeklyPlayerStats> (seeded empty)
